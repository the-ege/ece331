...
int ax25(char *callsign, char *info)
{
	...
	if (len_info > 256) return 1;
	buffer = (uint8_t*)malloc(25+len_info);
	...
	memcpy(buffer+INFO_IDX,info,len_info);
	
	//calculate crc on everything until the last byte of info,
	//which avoids the null terminator

	crc = calcCRC(buffer,INFO_IDX+len_info);
	buffer[INFO_IDX+len_info] = (crc >> 8);
	buffer[INFO_IDX+len_info+1] = crc;

	write(1,buffer,25+len_info);
