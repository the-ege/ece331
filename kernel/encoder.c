...
int encode(char *str, int len, int stuffFlag)
{
	...
	/* Program takes the string input and performs NRZI encoding on the
	string. Bitstuffing after five 1s is enabled if the stuffFlag is a 1. */
	...
	for (i=0;i<len;i++) {
		...
		for (j=0;j<8;j++) { //always dealing with 1B of data at a time
			...
			if (num != 0) { //the value is "1"
				...
				if ((stuffCount == 5) && (stuffFlag == 1)) { //see if we need to stuff a 0
					printf("%c ", state? 'S' : 'M');
